    ...
    public partial class Department : Form
    {
        ...
        private void dgvDepartments_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                if (e.RowIndex >= 0)
                {
                    txtDeptID.Text = dgvDepartments.Rows[e.RowIndex].Cells["DepartmentID"].Value.ToString();
                    txtDeptName.Text = dgvDepartments.Rows[e.RowIndex].Cells["DepartmentName"].Value.ToString();
                    txtDeptCode.Text = dgvDepartments.Rows[e.RowIndex].Cells["DepartmentCode"].Value.ToString();
                    cmbCollege.SelectedValue = dgvDepartments.Rows[e.RowIndex].Cells["CollegeID"].Value;
                    chkIsActive.Checked = Convert.ToBoolean(dgvDepartments.Rows[e.RowIndex].Cells["IsActive"].Value);
                }
            }
            catch (Exception ex)
            {
